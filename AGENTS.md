...
- **Never commit changes, create/update the changelog, or push to remote unless the user explicitly asks.** Complete the code/doc work first, then wait for the user to say "commit", "push", or "create changelog" before doing so.
- When the user does ask to commit: audit and update all affected documentation under `docs/` and `README.md` first — any change to directory structure, component paths, script behaviour, or page inventory must be reflected in the relevant docs. Key docs to check: `docs/quick-start/02-project-structure.md`, `docs/scripts/README.md`, `docs/reusable-implementations/`, and `README.md`.
- When the user does ask to commit: generate or update the changelog first — create `changelog/vX.Y.Z.md` following the Keep a Changelog format, bump the version appropriately (patch for fixes, minor for new features, major for breaking changes), and update `changelog/README.md` to list the new version as the latest release.
